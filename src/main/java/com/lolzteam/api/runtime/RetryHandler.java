        ...
        .exceptionallyCompose(
            error -> {
              ...
              if (!isRetryable(cause) || attempt >= config.maxRetries()) {
                ...
                return CompletableFuture.failedFuture(new RetryExhaustedError(attempt + 1, cause));
              }
              long delayMs = computeDelay(attempt, config, cause);
              if (onRetry != null) {
                var retryError =
                    cause instanceof LolzteamException le ? le : new NetworkException(cause);
                onRetry.accept(
                    new RetryInfo(
                        attempt + 1, Duration.ofMillis(delayMs), retryError, method, path));
              }
