...
class ConfigTest {
	...
	void builderCreatesConfig() {
		var config = ClientConfig.builder("my-token")
	...
	void proxyRejectsInvalidUrl() {
		assertThrows(ConfigException.class, () ->
			new LolzteamHttpClient(ClientConfig.builder("t")
				.baseUrl("https://example.com")
	...
	void proxyAcceptsHttpProxy() {
		var client = new LolzteamHttpClient(ClientConfig.builder("t")
			.baseUrl("https://example.com")
			.proxy(new ProxyConfig("http://proxy:8080"))
			.build());
		client.close();
