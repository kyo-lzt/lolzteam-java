...
class ClientTest {
	...
	private static final RetryConfig NO_RETRY = new RetryConfig(0, Duration.ofMillis(10), Duration.ofMillis(50));
	...
	private LolzteamHttpClient client(String token, RetryConfig retry) {
		return new LolzteamHttpClient(ClientConfig.builder(token)
			.baseUrl(baseUrl)
			.retry(retry)
			.build());
	}
	...
	void pathParameterSubstitution() {
		...
		server.createContext("/categories/42", exchange -> {
	...
	void forumClientCanBeInstantiated() {
		try (var forum = new ForumClient(ClientConfig.builder("test-token")
			.baseUrl(baseUrl)
			.retry(NO_RETRY)
			.build())) {
	...
	void marketClientCanBeInstantiated() {
		try (var market = new MarketClient(ClientConfig.builder("test-token")
			.baseUrl(baseUrl)
			.retry(NO_RETRY)
			.build())) {
